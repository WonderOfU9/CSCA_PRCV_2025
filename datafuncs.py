...
class ImageFolderWithRatingsAndFilenames_VARS(torchvision.datasets.ImageFolder):
    def __init__(self, root, transform, ratings,contents):
        super().__init__(root, transform)
        self.ratings = ratings
    ...
    def __getitem__(self, index):
        original_tuple = super(ImageFolderWithRatingsAndFilenames_VARS, self).__getitem__(index)
        path = self.imgs[index][0]
        fname = path.split("/")[-1]
        imgname = fname.split(".")[0]
        # fname = fname.split(".")[0]
        ...
        rating = self.ratings[np.where(self.ratings[:, 0] == imgname)][0][1]
        #style = self.styles[np.where(self.ratings[:, 0] == imgname)][0][1]
        content = self.contents[np.where(self.ratings[:, 0] == imgname)][0][1]


        new_tuple = (original_tuple + (rating,fname,content)) #按这个顺序返回每个batch的数据
        return new_tuple