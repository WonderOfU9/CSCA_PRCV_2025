...
class ImageFolderWithRatingsAndFilenames_CSCA(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        original_tuple = super(ImageFolderWithRatingsAndFilenames_CSCA, self).__getitem__(index)
        path = self.imgs[index][0]
        fname = path.split("/")[-1]
        imgname = fname.split(".")[0]
        # fname = fname.split(".")[0]
